//
//  StandTraining_ViewController.swift
//  FypTest_APP
//
...
class StandTraining_ViewController: UIViewController {
    let standTraining_videoCapture = StandTraining_VideoCapture()
    ...
    override func viewDidLoad() {
        ...
        setupVideoPreview()
        //pose detection
        standTraining_videoCapture.standTraining_Predictor.delegate = self
    }
                ...
                if((Actioncount == User_ActionAmount)&&(TrainSetCount < User_TrainSetAmount)){
                    ...
                    //show alert & save data to firebase
                        ...
                        db.collection("Record").document(user.uid).collection("data").document("\(self.titleLBL.text!) \(String(time2))").setData([
                                    ...
                                    "Record_URL": "Record/Icon_50_backMuscles@3x.png"
                                ])
                            //show alertf
                            showAlertF()
                    }
    ...
    private func setupVideoPreview(){
        standTraining_videoCapture.startCaptureSession()
        previewLayer = AVCaptureVideoPreviewLayer(session: standTraining_videoCapture.captureSession)
...
extension StandTraining_ViewController: StandTrainingDelegte{
    func StandTraining(standTraining_predictor: StandTraining_Predictor, didLableAction action: String, with confience: Double) {
        ...
        if action == "BicepsCorrect" && confience > 0.70 && isThrowDetected == false{
            ...
            DispatchQueue.main.async {
                ...
                //action detected play sound to alert user
