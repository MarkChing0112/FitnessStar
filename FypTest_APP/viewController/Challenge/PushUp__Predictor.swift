...
typealias ThrowingClassifies_C_1 = Pushup

protocol Chest_ChallengeDelegte: AnyObject {
...
class Chest_Predictor {
    weak var  delegate: Chest_ChallengeDelegte?
    ...
    func  labelActionType(){
        ...
        delegate?.Chest_Challenge(chest_Challenge_predictor: self, didLableAction: label, with: confience)
    ...
    func processObservation( observation: VNHumanBodyPoseObservation){
         do{
            ...
            delegate?.Chest_Challenge(chest_Challenge_predictor: self, didFindNewRecognizedPoints: displayPoints)
        }
         catch {
