...
class Stand_Predictor {
    weak var  delegate: Stand_ChallengeDelegte?
    ...
    func  labelActionType(){
        guard let throwingClassifier4 = try? ThrowingClassifies_C_3(configuration:         MLModelConfiguration()),
        ...
        delegate?.Stand_Challenge(stand_Challenge_predictor: self, didLableAction: label, with: confience)
    ...
    func processObservation( observation: VNHumanBodyPoseObservation){
         do{
            ...
            delegate?.Stand_Challenge(stand_Challenge_predictor: self, didFindNewRecognizedPoints: displayPoints)
        }
         catch {
