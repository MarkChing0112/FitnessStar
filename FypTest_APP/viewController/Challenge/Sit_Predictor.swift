...
typealias ThrowingClassifies_C_2 = Sit

protocol Sit_ChallengeDelegte: AnyObject {
...
class Sit_Predictor {
    weak var  delegate: Sit_ChallengeDelegte?
    ...
    func  labelActionType(){
        ...
        delegate?.Sit_Challenge(sit_Challenge_predictor: self, didLableAction: label, with: confience)
    ...
    func processObservation( observation: VNHumanBodyPoseObservation){
         do{
            ...
            delegate?.Sit_Challenge(sit_Challenge_predictor: self, didFindNewRecognizedPoints: displayPoints)
        }
         catch {
